...
//     if (selector.toString().includes('state.user.status')) return true; // Change to false to test logged out state
...
import React, { useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import { FaClock, FaListOl, FaLock } from 'react-icons/fa';
import { useSelector, useDispatch } from 'react-redux';
import useAxios from '../Utils/useAxios';
import { useWatchLaterToggle } from '../Utils/useWatchLaterToggle';
import { setSavedVideoIds } from '../features/Video/watchLaterSlice.js';

function WatchLaterPage() {
  const dispatch = useDispatch();
  const { sendRequest } = useAxios();
  const { toggleSaveStatus } = useWatchLaterToggle();

  const isLoggedIn = useSelector(state => state.user.status);
  const watchLaterId = useSelector(state => state.watchLater.watchLaterPlaylistId);
  const savedVideoIds = useSelector(state => state.watchLater.savedVideoIds);

  const [watchLaterVideos, setWatchLaterVideos] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // Fetch Watch Later playlist videos
  const fetchWatchLaterVideos = async () => {
    ...
    try {
      const res = await sendRequest({
        method: 'get',
        url: `/playlist/${watchLaterId}`,
      });

      // Assuming your backend returns playlist with `videos` array
      const videos = res.data.videos || [];
      setWatchLaterVideos(videos);
      // Update Redux savedVideoIds in case user refreshes page
      dispatch(setSavedVideoIds(videos.map(v => v._id)));
    } catch (err) {
      console.error('Failed to fetch Watch Later videos:', err);
      setError('Failed to load Watch Later videos. Please try again.');
      setWatchLaterVideos([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (isLoggedIn && watchLaterId) {
      fetchWatchLaterVideos();
    }
  }, [isLoggedIn, watchLaterId]);

  // Handle remove from Watch Later
  const handleRemove = async (videoId) => {
    try {
      await toggleSaveStatus(videoId); // This will remove the video
      setWatchLaterVideos(prev => prev.filter(v => v._id !== videoId));
    } catch (err) {
      ...
      <div className="space-y-4">
        {watchLaterVideos.map((video) => (
          ...
          >
            ...
            <div className="flex flex-col flex-grow">
              <Link to={`/video/${video._id}`} className="text-lg font-semibold line-clamp-2 text-gray-900 dark:text-white hover:text-blue-500">
                {video.title}
              </Link>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{video.channelName}</p>
              <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
            ...
            <button 
              className="text-gray-400 hover:text-red-500 transition duration-150 p-2 flex-shrink-0"
